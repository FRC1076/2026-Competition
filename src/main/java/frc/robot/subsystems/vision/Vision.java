...
public class Vision extends SubsystemBase {
  private final AprilTagLayoutType aprilTagLayoutSupplier;
  ...
  public Vision(AprilTagLayoutType aprilTagLayoutSupplier, VisionIO... io) {
  ...
  public void periodic() {
    ...
    for (int instanceIndex = 0; instanceIndex < io.length; instanceIndex++) {
      ...
      for (int frameIndex = 0;
        ...
        double[] values = aprilTagInputs[instanceIndex].frames[frameIndex];
        Optional<Pose3d> robotToCamera = cameras[instanceIndex].poseFunction().apply(timestamp);
        ...
        for (int i = (values[0] == 1 ? 9 : 17); i < values.length; i += 10) {
          ...
          Optional<Pose3d> tagPose =
              aprilTagLayoutSupplier.getLayout().getTagPose((int) values[i]);
