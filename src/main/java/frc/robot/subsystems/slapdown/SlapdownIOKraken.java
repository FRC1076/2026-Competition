...
public class SlapdownIOKraken implements SlapdownIO {
    ...
    private final PositionVoltage m_positionRequestStrong;
    ...
    public SlapdownIOKraken() {
        ...
        // Weak PID
        m_motorConfig.Slot1.kP = m_unitConverter.fromSIkP(SlapdownConstants.kPStrong);
        ...
        m_positionRequest = new MotionMagicVoltage(0)
            .withEnableFOC(SlapdownConstants.kUseFOC);
        m_positionRequestStrong = new PositionVoltage(0)
            .withEnableFOC(SlapdownConstants.kUseFOC);
    ...
    @Override
    public void setPositionStrong(double positionRadians) {
        m_positionRequestStrong.withPosition(m_unitConverter.fromSIPos(positionRadians));
        m_motor.setControl(m_positionRequestStrong);
    }
    ...
    public void updateInputs(SlapdownIOInputs inputs) {
        ...
        var activeControl = m_motor.getAppliedControl();
        if (activeControl instanceof PositionVoltage pos) {
            inputs.PIDActive = true;
            inputs.PIDTargetRadians = m_unitConverter.toSIPos(pos.Position);
        } else if (activeControl instanceof MotionMagicVoltage mmv) {
            inputs.PIDActive = true;
            inputs.PIDTargetRadians = m_unitConverter.toSIPos(mmv.Position);
        } else {
            inputs.PIDActive = false;
            inputs.PIDTargetRadians = 0;
        }
    }
}
