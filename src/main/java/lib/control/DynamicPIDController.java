...
public class DynamicPIDController implements Sendable, AutoCloseable {
    ...
    // The periodSupplier (in seconds) of the loop that calls the controller
    private final DoubleSupplier m_periodSupplier;

    private final double m_minPeriod;

    private final double m_maxPeriod;
    ...
    /**
     * Allocates a PIDController with the given constants for kp, ki, and kd and a default periodSupplier of
     * 0.02 seconds.
     *
     ...
     */
    public DynamicPIDController(double kp, double ki, double kd) {
        this(kp, ki, kd, () -> 0.02, 0.02, 0.02);
    }
     ...
     * @param ki The integral coefficient.
     * @param kd The derivative coefficient.
     * @param periodSupplier The periodSupplier between controller updates in seconds.
     * @param minPeriod The minimum period between controller updates in seconds.
     * @param maxPeriod The maximum period between controller updates in seconds.
     * @throws IllegalArgumentException if kp &lt; 0
     * @throws IllegalArgumentException if ki &lt; 0
     * @throws IllegalArgumentException if kd &lt; 0
     * @throws IllegalArgumentException if periodSupplier == null
     */
    @SuppressWarnings("this-escape")
    public DynamicPIDController(double kp, double ki, double kd, DoubleSupplier periodSupplier, double minPeriod, double maxPeriod) {
        m_kp = kp;
        m_ki = ki;
        m_kd = kd;
        ...
        if (minPeriod < 0.0) {
            throw new IllegalArgumentException("Minimum period must be a non-negative number!");
        }

        if (maxPeriod < 0.0) {
            throw new IllegalArgumentException("Maximum period must be a non-negative number!");
        }

        if (minPeriod > maxPeriod) {
            throw new IllegalArgumentException("Minimum period must be less than or equal to maximum period!");
        }
        ...
        if (periodSupplier == null) {
            throw new IllegalArgumentException("Controller periodSupplier must be a positive number!");
        }
        m_periodSupplier = periodSupplier;
    ...
    /**
     * Returns the periodSupplier of this controller.
     *
     * @return the periodSupplier of the controller.
     */
    public DoubleSupplier getPeriodSupplier() {
        return m_periodSupplier;
    }
    ...
    public double getPeriod() {
        double period = m_periodSupplier.getAsDouble();
        period = MathUtil.clamp(period, m_minPeriod, m_maxPeriod);
        return period;
    }
    ...
    public void setSetpoint(double setpoint) {
        ...
        m_errorDerivative = (m_error - m_prevError) / getPeriod();
    }
    ...
    public double calculate(double measurement) {
        ...
        if (Math.abs(m_error) > m_iZone) {
            ...
            m_totalError =
                MathUtil.clamp(
                    m_totalError + m_error * getPeriod(),
